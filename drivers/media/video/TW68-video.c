...
{ 
	...
	if (res_check(fh,RESOURCE_VIDEO)) {
		...
		//  printk( "TW68 RESOURCE_VIDEO %s, ch=%d\n", __func__,fh->DMA_nCH); // liutest
		//mutex_lock(&fh->cap.vb_lock);
						...
						struct v4l2_format *f)
{
	...
	printk( "TW68 input  nId:%x   try_fmt:: %x \n", nId, f->fmt.pix.pixelformat );
	...
	v4l_bound_align_image(&f->fmt.pix.width, 48, maxw, 2,    // 4 pixel  test 360  4,
...
// pci_alloc_consistent   32 4 * 8  continuous field memory buffer
	...
	// mod_timer(&dev->delay_resync, jiffies+ msecs_to_jiffies(30)); // liutest remove resync timer, replace with tasklet 
	...
	for (k=0; k<9; k++)
	{
		...
		// liutest add
		mutex_init(&dev->video_dmaq[k].lock);
...
void TW68_irq_video_done(struct TW68_dev *dev, unsigned int nId, u32 dwRegPB)
{
	...
	int Fn, PB;   /// d0, w0, 
		...
		{
			 //printk("liutest  TW68__irq_video_done QF_Field_Copy \n");
	...
	//mutex_lock(&(dev->video_dmaq[nId].lock));  // liutest add

	//printk("liutest TW68__irq_video_done .curr%p    nId= 0X%x    dwRegPB 0X%X \n", dev->video_dmaq[nId].curr, nId, dwRegPB);
	...
	/// printk("TW68__irq_video_done video_dmaq[nId].curr   nId  %x  dwRegPB 0x%X \n", nId, dwRegPB);
		...
		Fn = (dwRegPB >>24) & (1<< (nId-1));
		PB = (dwRegPB ) & (1<< (nId-1));
		//printk(" liutest TW68__irq_video_done    Fn:%d   PB:%d \n", Fn, PB);
		...
		*/
		...
		if (Fn ==0)
		{
			// field 0 interrupt  program update  P field mapping
			   ...
			   //printk(" liutest TW68__irq_video_done  BF_Copy Fn 0,   Done:%d   PB:%d \n", Done, PB);
			//Done =1;
		...
		else
		{
			...
			if (Done)
				...
				//printk(" liutest TW68__irq_video_done  BF_Copy Fn 1,   Done:%d   PB:%d \n", Done, PB);
			...
			//TW68_buffer_next(dev,&(dev->video_dmaq[nId]));  // liutest

			//printk(" liutest TW68__irq_video_done  after finish  fn1 copy Done:%d  \n", Done);
		}
	}
	// B field interrupt  program update  P field mapping
	 TW68_buffer_next(dev,&(dev->video_dmaq[nId]));  // liutest move to finish
	//printk("\n");

 done:
	// mutex_unlock(&(dev->video_dmaq[nId].lock));  // liutest add
	return;
}
